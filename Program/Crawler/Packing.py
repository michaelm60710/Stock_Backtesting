...
def Concate_df(path, dateparse_str = '%Y/%m/%d', date_name = '日期'):
    dateparse = lambda x: pd.datetime.strptime(x, dateparse_str)
    dfs = []
    for file in  os.listdir(path):
        if not file.endswith(".csv"): continue
...
def Check_miss_data():
    ...
    for func_name, value in Crawl_dict.items():
        ...
        func_index             = origin_dict[func_name].index.drop_duplicates()
        d_index = d_index.union(func_index)

        #print(func_name)
        #print("    # index length: " + str(len(func_index) ) )
